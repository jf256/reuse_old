...
#  if (every2grid){
#    load(file=paste0('../data/analysis/analysis_',cyr,'_2ndgrid.Rdata'))
#  } else {  
...
#  }
  ...
  # 1. Analysis indices
    ech_ind <- echam.anom
    ech_ind$data <- echam.anom$data[which(echam.anom$names=="DIMI" | echam.anom$names=="z100" | echam.anom$names=="z300" |
                       echam.anom$names=="PWC" | echam.anom$names=="HC" | echam.anom$names=="SJ"),,]
    ech_ind$ensmean <- echam.anom$ensmean[which(echam.anom$names=="DIMI" | echam.anom$names=="z100" | echam.anom$names=="z300" |
                                       echam.anom$names=="PWC" | echam.anom$names=="HC" | echam.anom$names=="SJ"),]
    ech_ind$names <- echam.anom$names[which(echam.anom$names=="DIMI" | echam.anom$names=="z100" | echam.anom$names=="z300" |
                      echam.anom$names=="PWC" | echam.anom$names=="HC" | echam.anom$names=="SJ")]
    ech_ind$lon <- echam.anom$lon[which(echam.anom$names=="DIMI" | echam.anom$names=="z100" | echam.anom$names=="z300" |
                                         echam.anom$names=="PWC" | echam.anom$names=="HC" | echam.anom$names=="SJ")]
    ech_ind$lat <- echam.anom$lat[which(echam.anom$names=="DIMI" | echam.anom$names=="z100" | echam.anom$names=="z300" |
                                         echam.anom$names=="PWC" | echam.anom$names=="HC" | echam.anom$names=="SJ")]
...
#      analysis.anom$data <- apply(atmp,c(1,3,4),mean)
  ...
  if (save_prepplot){
    if (vali) {
      print(paste("dim validate data:",dim(validate$data)))
      save(analysis,analysis.anom,ana_ind,echam,echam.anom,ech_ind,validate,vali_ind,calibrate,
           file=paste0('../data/prepplot/analysis_',cyr,'.Rdata'))
    } else {
...
#    # write entire analysis to netcsf format
    # if (write_netcdf) {
    #   #for v in unique(echam$names) {} to export all vars and not only temp2
    #   outpos <- which(echam$names=="temp2")
    #   out <- analysis$ensmean[outpos,]
    #   out[out==NA] <- -999 #set missing value to -999
    #   #define dimensions
    #   unilon <- unique(analysis$lon)
    #   unilat <- unique(analysis$lat)
    #   # create full 3D field with potential NA over ocean
    #   fullout <- array(NA,dim=c(length(unilon),length(unilat),ncol(out)))
    #   for (t in 1:ncol(out)) {
    #     for (i in 1:length(unilon)) {
    #       for (j in 1:length(unilat)) {
    #         #            #print(c(i,j,t))
    #         pos <- NA
    #         pos <- intersect(which(analysis$lon[outpos]==unilon[i]),
    #                          which(analysis$lat[outpos]==unilat[j]))
    #         if (!is.na(pos)){
    #           fullout[i,j,t] <- out[pos,t]
    #         }
    #       }
    #     }
    #   }
    #   # define dimensions  
    #   x <- ncdim_def( "lon", "degrees_E", vals=unilon)
    #   y <- ncdim_def( "lat", "degrees_N", vals=unilat)
    #   time <- ncdim_def("time", "months since 1600-01-15 00:00:00", 
    #                     vals=((cyr-1600)*12-3):((cyr-1600)*12+8))
    #   temp2 <- ncvar_def(name="temp2",longname="temperature at 2m", 
    #                     units="degree celsius", dim=list(x,y, time), missval=-999)
    #   outfile <- nc_create(filename="../data/netcdf/temp2_analysis.nc", vars=list(temp2))
    #   ncvar_put(outfile, temp2, fullout) # write values to file and close it
    #   # something is wrong: no southern hemisphere data in last timestep of nc file
    #   # although fullout looks fine
    # }
...
if (every2grid) {
  save.image(file=paste("../data/prepplot_validation_image_",syr,"-",eyr,"_2ndgrid.Rdata",sep=""))
} else {
  save.image(file=paste("../data/prepplot_validation_image_",syr,"-",eyr,".Rdata",sep=""))
}
} # end (load_1902_2001){
...
if (load_image){
  if (every2grid) {
    load(file=paste("../data/prepplot_validation_image_",syr,"-",eyr,"_2ndgrid.Rdata",sep=""))
  } else {
    load(file=paste("../data/prepplot_validation_image_",syr,"-",eyr,".Rdata",sep=""))
  }
}
...
a.tmp <- array(analysis$data[,1:160,],dim=c(dim(analysis$data[,1:160,])[1],2,
            (dim(analysis$data[,1:160,])[2]/2), dim(analysis$data[,1:160,])[3])) 
...
for (i in which(analysis$names=="temp2")) {
...
FairSprErr2 <- function (ens, obs, obs.sd=NULL) {
  ...
  if (is.null(obs.sd)) {
    ...
    error <- sqrt((obs - rowMeans(ens))^2)-(obs.sd**2)
    # RMSE_t = sqrt(RMSE**2 - o_sd**2)
  }
