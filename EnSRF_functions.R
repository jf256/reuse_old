      ...
      for (varname in c('temp2', 'precip', 'slp', 'geopoth', 'u', 'v', 'omega',
        ...
        if (varname %in% names(nc$var)){
          ...
          if (varname=='st') {     
            ...
            print(dim(data))
            outdata <- rbind(outdata, data)
            names <- c(names, c(rep('t500',nrow(data))))
            #          print(names)
          }
          if (varname=='stream') {     
            data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)),
...
read_echam3 <- function(filehead, path=echallvarpath, xlim=c(-180,180), ylim=c(-90,90), timlim=c(1600, 2005), small=F, landonly=F){
  ...
  for (f in files){
    ...
    for (varname in c('temp2', 'precip', 'slp', 'geopoth', 'u', 'v', 'omega', 'st', 'stream')){
#      'geopoth' 1=50000 2=10000
#      'u', 'v' 1=85000 2=20000
#       'omega' 1=50000
#      'st' 1=85000
#      'stream' 5 lev but just 1 lon (zonal mean) 100000, 85000, 50000, 30000, 20000
      ...
      if (varname %in% names(nc$var)){
        if ((varname=='temp2') || (varname=='precip') || (varname=='slp')) { 
          if (length(nc$var[[varname]]$dim) == 3){
            data <- ncvar_get(nc, varname, start=c(1,1,min(ti)), count=c(-1,-1,length(ti)))[loi, lai,]
          } else if (length(nc$var[[varname]]$dim) == 4){
            data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), count=c(-1,-1,1,length(ti)))[loi, lai,]
            length(nc$var[[varname]]$dim[[3]]$vals)
          }
          if (landonly){
            data <- array(data, c(length(lsm), dim(data)[3]))[lsm > 0.5, ]
...
#          print(names)
  ...
  #          print(names)
        ...
        if (varname=='stream') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), count=c(1,-1,1,length(ti)))[laistream,]
#          data <- array(data, c(dim(data)[1]*dim(data)[2], dim(data)[3])) 
          print(dim(data))
          outdata <- rbind(outdata, data)
          names <- c(names, rep(varname, nrow(data)))
...
# NEW read in the ensemble of echam CCC400 simulations (T, P, SLP, wind, stream, gph, ...)
read_echam2 <- function(filehead, path=echallvarpath, xlim=c(-180,180), ylim=c(-90,90), timlim=c(1600, 2005),small=F){
  ...
  for (f in files){
    ...
    loi <- which(lon >= xlim[1] & lon <= xlim[2])
    lai <- which(lat >= ylim[1] & lat <= ylim[2])
    lonstream <- nc$dim$lon_2$vals
    latstream <- nc$dim$lat_2$vals
    lonstream[lonstream > 180] <- lonstream[lonstream > 180] - 360
    loistream <- which(lonstream >= xlim[1] & lonstream <= xlim[2])
    laistream <- which(latstream >= ylim[1] & latstream <= ylim[2])
    longph300 <- nc$dim$lon_2$vals
    latgph300 <- nc$dim$lat_3$vals
    longph300[longph300 > 180] <- longph300[longph300 > 180] - 360
    loigph300 <- which(longph300 >= xlim[1] & longph300 <= xlim[2])
    laigph300 <- which(latgph300 >= ylim[1] & latgph300 <= ylim[2])    
    longph100 <- nc$dim$lon_2$vals
    latgph100 <- nc$dim$lat_4$vals
    longph100[longph100 > 180] <- longph100[longph100 > 180] - 360
    loigph100 <- which(longph100 >= xlim[1] & longph100 <= xlim[2])
    laigph100 <- which(latgph100 >= ylim[1] & latgph100 <= ylim[2])
    longph100_2 <- nc$dim$lon_2$vals
    latgph100_2 <- nc$dim$lat_5$vals
    longph100_2[longph100_2 > 180] <- longph100_2[longph100_2 > 180] - 360
    loigph100_2 <- which(longph100_2 >= xlim[1] & longph100_2 <= xlim[2])
    laigph100_2 <- which(latgph100_2 >= ylim[1] & latgph100_2 <= ylim[2])
    lonu200 <- nc$dim$lon_2$vals
    latu200 <- nc$dim$lat_6$vals
    lonu200[lonu200 > 180] <- lonu200[lonu200 > 180] - 360
    loiu200 <- which(lonu200 >= xlim[1] & lonu200 <= xlim[2])
    laiu200 <- which(latu200 >= ylim[1] & latu200 <= ylim[2])
    lonomega <- nc$dim$lon_3$vals
    latomega <- nc$dim$lat_7$vals
    lonomega[lonomega > 180] <- lonomega[lonomega > 180] - 360
    loiomega <- which(lonomega >= xlim[1] & lonomega <= xlim[2])
    laiomega <- which(latomega >= ylim[1] & latomega <= ylim[2])
    lonomega_2 <- nc$dim$lon_4$vals
    latomega_2 <- nc$dim$lat_7$vals
    lonomega_2[lonomega_2 > 180] <- lonomega_2[lonomega_2 > 180] - 360
    loiomega_2 <- which(lonomega_2 >= xlim[1] & lonomega_2 <= xlim[2])
    laiomega_2 <- which(latomega_2 >= ylim[1] & latomega_2 <= ylim[2])   
    if (small==T) {
      # mulc for reading each 3rd grid cell to avoid memory problems
      loi <- loi[seq(ceiling(mulc/2),length(loi),mulc)]
      lai <- lai[seq(ceiling(mulc/2), length(lai),mulc)]
      loistream <- loistream[seq(ceiling(mulc/2),length(loistream),mulc)]
      laistream <- laistream[seq(ceiling(mulc/2), length(laistream),mulc)]
      loigph100 <- loigph100[seq(ceiling(mulc/2),length(loigph100),mulc)]
      laigph100 <- laigph100[seq(ceiling(mulc/2), length(laigph100),mulc)]
      loigph100_2 <- loigph100_2[seq(ceiling(mulc/2),length(loigph100_2),mulc)]
      laigph100_2 <- laigph100_2[seq(ceiling(mulc/2), length(laigph100_2),mulc)]
      loigph300 <- loigph300[seq(ceiling(mulc/2),length(loigph300),mulc)]
      laigph300 <- laigph300[seq(ceiling(mulc/2), length(laigph300),mulc)]
      loiu200 <- loiu200[seq(ceiling(mulc/2),length(loiu200),mulc)]
      laiu200 <- laiu200[seq(ceiling(mulc/2), length(laiu200),mulc)]
      loiomega <- loiomega[seq(ceiling(mulc/2),length(loiomega),mulc)]
      laiomega <- laiomega[seq(ceiling(mulc/2), length(laiomega),mulc)]
      loiomega_2 <- loiomega_2[seq(ceiling(mulc/2),length(loiomega_2),mulc)]
      laiomega_2 <- laiomega_2[seq(ceiling(mulc/2), length(laiomega_2),mulc)]
    }
    ...
    outdata <- NULL
    names <- NULL
    for (varname in c('temp2', 'precip', 'slp', 'stream', 'gph100', 'gph100_2', 'gph300', 'u200', 'omega', 'omega_2')){
      print(varname)
      if (varname %in% names(nc$var)){
        if ((varname=='temp2') || (varname=='precip') || (varname=='slp')) { 
          if (length(nc$var[[varname]]$dim) == 3){
        ...
        if (varname=='stream') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                        count=c(-1,-1,1,length(ti)))[loistream, laistream,]
        }  
        if (varname=='gph100') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                      count=c(-1,-1,1,length(ti)))[loigph100, laigph100,]
        }  
        if (varname=='gph100_2') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                      count=c(-1,-1,1,length(ti)))[loigph100_2, laigph100_2,]
        }  
        if (varname=='gph300') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                      count=c(-1,-1,1,length(ti)))[loigph300, laigph300,]
        }  
        if (varname=='u200') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                      count=c(-1,-1,1,length(ti)))[loiu200, laiu200,]
        }  
        if (varname=='omega') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                      count=c(-1,-1,1,length(ti)))[loiomega, laiomega,]
        }  
        if (varname=='omega_2') {     
          data <- ncvar_get(nc, varname, start=c(1,1,1,min(ti)), 
                        count=c(-1,-1,1,length(ti)))[loiomega_2, laiomega_2,]
        } 
        if ((varname!='temp2') && (varname!='precip') && (varname!='slp')) {
          data <- array(data, c(dim(data)[1]*dim(data)[2], dim(data)[3]))
        }
...
#     glob.mn <- (lats > -60) * cos(lats/180*pi)
#     glob.mn <- glob.mn/sum(glob.mn)
#     eu.mn <- (lats > 37 & lats < 75 & lons > -15 & lons < 35)*cos(lats/180*pi)
#     eu.mn <- eu.mn/sum(eu.mn)
#     n.mn <- nrow(outdata) %/% length(lons) 
#     H.mn <- array(0, c(2*n.mn, nrow(outdata)))
#     for (i in 1:n.mn){
#       H.mn[i,i*length(lons) - length(lons):1 + 1] <- glob.mn
#       H.mn[n.mn+i,i*length(lons) - length(lons):1 + 1] <- eu.mn
#     }
#     outdata <- rbind(outdata, H.mn %*% outdata)
#     # check whether all of temp2, precip, and slp are available
#     add.names <- as.vector(t(outer(c('glob', 'eu'), unique(names[seq(1,n.mn*length(lons))]), paste, sep='.')))
...
#     #time <- apply(array((tim[ti])[11:(length(ti)-2)], c(6, length(ti)/6 -2)), 2, mean, na.rm=T)
  ...
  data <- list(data=array(unlist(tmp), c(dim(tmp[[1]]), length(files))),
               ensmean=ensmean, lon=rep(lon[loi], length(lai))[lsm > 0.5],
               lat=rep(lat[lai], each=length(loi))[lsm > 0.5], 
               lonstream=rep(lonstream[loistream], length(laistream)), 
               latstream=rep(latstream[laistream], each=length(loistream)),
               longph100=rep(longph100[loigph100], length(laigph100)),
               latgph100=rep(latgph100[laigph100], each=length(loigph100)),
               longph100_2=rep(longph100_2[loigph100_2], length(laigph100_2)),
               latgph100_2=rep(latgph100_2[laigph100_2], each=length(loigph100_2)),
               longph300=rep(longph300[loigph300], length(laigph300)),
               latgph300=rep(latgph300[laigph300], each=length(loigph300)),
               lonu200=rep(lonu200[loiu200], length(laiu200)),
               latu200=rep(latu200[laiu200], each=length(loiu200)),
               lonomega=rep(lonomega[loiomega], length(laiomega)),
               latomega=rep(latomega[laiomega], each=length(loiomega)),
               lonomega_2=rep(lonomega_2[loiomega_2], length(laiomega_2)),
               latomega_2=rep(latomega_2[laiomega_2], each=length(loiomega_2)),
               height=alt[lsm > 0.5], lsm.i=which(lsm > 0.5),
               time=time, names=names)
  invisible(data) 
}
...
# read_echam_ind <- function(filehead, path=echindpath, xlim=c(-180,180), ylim=c(-90,90), timlim=c(1600, 1630)){
# 
#   files <- list.files(path, pattern=paste('^', filehead, sep=''), full.names=T)
#   tmp <- list()
#   for (f in files){
...
read_echam1 <- function(filehead, path=echpath, xlim=c(-180,180), ylim=c(-90,90), timlim=c(1600, 1630), small=F, landonly=F){
  ...
  # mask out sea grid boxes (no variability)
  if (landonly){
    nc <- nc_open(paste(echpath, '../landseamask.nc', sep='/'))
    nc2 <-nc_open(paste(echpath, '../orography.nc', sep="/"))
    lon <- nc$dim$lon$vals
    lat <- nc$dim$lat$vals
    lon[lon > 180] <- lon[lon > 180] - 360
    loi <- which(lon >= xlim[1] & lon <= xlim[2])
...
# read in CRU
# read_cru <- function(path='~/unibe/data/cru', timlim=c(syr,eyr), xlim=c(-180,180), ylim=c(-90,90)){
#   if (timlim[1]<1850) {
#     print('Error: CRUTEM3v starts in 1850')
#   } else {
#   f <- list.files(path, pattern='cru_echamgrid', full.names=T)
#   nc <- nc_open(f)
#   lon <- nc$dim$lon$vals
#   lat <- nc$dim$lat$vals
#   loi <- which(lon < xlim[2] | lon > 360 + xlim[1])
#   lai <- which(lat > ylim[1] & lat < ylim[2])
# #  tim <- ncdf_times(nc)
# #  ti  <- which(tim > as.Date(paste(timlim[1],'-01-01',sep='')) & tim < as.Date(paste(timlim[2],'-01-01',sep='')))
#   year <- as.numeric(format(ncdf_times(nc), "%Y"))
#   month <- as.numeric(format(ncdf_times(nc), "%m"))
#   tim <- year + (month-0.5)/12
#   ti <- which(tim >= timlim[1] & tim < (timlim[2]+1))
#   time <- tim[ti]
#   data <- ncvar_get(nc, start=c(1,1,1,min(ti)), count=c(-1,-1,-1,length(ti)))[loi, lai,]
#   lon[lon > 180] <- lon[lon > 180] - 360
#   lon <- lon[loi]
#   lat <- lat[lai]
#   loi <- sort(lon, index=TRUE)$ix
#   lai <- sort(lat, index=TRUE)$ix
# #  data <- array(data[loi, lai,], c(length(loi)*length(lai), dim(data)[3]))
#   data <- list(data=array(data[loi, lai,], c(length(loi)*length(lai), dim(data)[3])), lon=rep(lon[loi], length(lai)), lat=rep(lat[lai], each=length(loi)), time=time) 
#   invisible(data)  
#   }
# }
# read in the ensemble of echam castro simulations
# read_cru <- function(filehead, path=echpath, xlim=c(-180,180), ylim=c(-90,90), timlim=c(1600, 1630)){
# 
#   # read in the land-sea mask of echam
#   # mask out sea grid boxes (no variability)
#   nc <- nc_open(paste(echpath, '../landseamask.nc', sep='/'))
...
#   loi <- which(lon >= xlim[1] & lon <= xlim[2])
#   lai <- which(lat >= ylim[1] & lat <= ylim[2])
# # only read every 3 grid cell to save memory
...
#         data <- array(data, c(length(lsm), dim(data)[3]))[lsm > 0.5, ]
# #        if (varname == 'temp2') data <- data - 273.15
# # CRUTEM is already in anomalies wrt 1961-1990
#         if (varname == 'temp2') data <- data
#         if (varname == 'precip') data <- data * 3600 * 24 * 30
#         if (varname == 'slp') data <- data / 100
#         # compute seasonal averages
#         #dat <- array(data[,11:(ncol(data)-2)], c(nrow(data), 6, ncol(data)/6 - 2))
...
# #  ensmean <- ensmean/(length(tmp)-1)
# 
# data <- list(data=outdata,
#                lon=rep(lon[loi], length(lai))[lsm > 0.5],
#                lat=rep(lat[lai], each=length(loi))[lsm > 0.5],
#                height=alt[lsm > 0.5], lsm.i=which(lsm > 0.5),
#                time=time, names=names)
# 
...
# # read in Luterbacher temperature reconstruction
# read_lut <- function(path='~/unibe/data/luterbacher/', timlim=c(syr,eyr), xlim=c(-25,40), ylim=c(35,70)){
#   if (timlim[1]<1659) {
#     print('Error: Luterbacher monthly resolved starts in 1659')
#   } else {
#   f <- list.files(path, pattern='TT_Luterbacher_monthly', full.names=T)
#   nc <- nc_open(f)
#   lon <- nc$dim$lon$vals
#   lat <- nc$dim$lat$vals
#   loi <- which(lon < xlim[2] | lon > 360 + xlim[1])
#   lai <- which(lat > ylim[1] & lat < ylim[2])
#   yrs=trunc(seq(1659,1998+(11/12),1/12))
#   mns=rep(seq(1,12,1),times=length(yrs)/12)  
#   dys=rep(1,times=length(mns))
#   tim=as.Date(paste(yrs,mns,dys,sep='-'))
#   ti  <- which(tim > as.Date(paste(timlim[1],'-01-01',sep='')) & tim < as.Date(paste(timlim[2],'-01-01',sep='')))
#   data <- ncvar_get(nc, start=c(1,1,min(ti)), count=c(-1,-1,length(ti)))[loi, lai,]
#   lon[lon > 180] <- lon[lon > 180] - 360
#   lon <- lon[loi]
#   lat <- lat[lai]
#   loi <- sort(lon, index=TRUE)$ix
...
# read in the corresponding NCEP/NCAR reanalysis
# read_ncep <- function(path='/net/schimper/lhome/bhendj/NNR', xlim=c(-180,180), ylim=c(0,90)){
# 
#   f <- list.files(path, pattern='T30.t2m', full.names=T)
#   nc <- nc_open(f)
#   lon <- nc$dim$lon$vals
#   lat <- nc$dim$lat$vals
#   loi <- which(lon < xlim[2] | lon > 360 + xlim[1])
#   lai <- which(lat > ylim[1] & lat < ylim[2])
#   tim <- ncdf_times(nc)
#   ti  <- which(tim > as.Date('1950-01-01') & tim < as.Date('2000-01-01'))
#   data <- ncvar_get(nc, start=c(1,1,min(ti)), count=c(-1,-1,length(ti)))[loi, lai,]
#   lon[lon > 180] <- lon[lon > 180] - 360
#   lon <- lon[loi]
#   lat <- lat[lai]
...
compute_avg_RE_pseudoproxy <- function(H, ech,ana,val){
    ...
    RE.mat <- array(0, c(nrow(H), 2, 30))
    RE.mat[,,1:29] <- RE$data
    RE.mat[,,30] <- RE$ensmean
    return(RE.mat)
}
...
# land sea mask to take care of missing validata in the ocean
    ...
    ana.rmse <- rmse_fun(ana.mn, valmn, seas=2)
    RE <- RE_fun(ana.rmse, ech.rmse)
    RE.mat <- array(0, c(nrow(H), 2, 31))
    RE.mat[,,1:30] <- RE$data
    RE.mat[,,31] <- RE$ensmean
    return(RE.mat)
}
...
#  moving northward from the latitude of maximum Ψ.
...
getgridboxnum <- function(var, echam) {
  m <- rep(NA,length(var$lon))
  echamatprox.arr <- array(NA,c(length(var$lon), length(var$time)))
...
#      print("mtmp > 0")
      m[i] <- mtmp
      try(if (var$names[i]=="precip") { 
        m[i] <- m[i]+length(echam$lon) 
      } else if (var$names[i]=="slp") { 
        m[i] <- m[i]+(2*length(echam$lon)) 
      },silent = T)
    } else { m[i] <- NA }
  }
  invisible(m)
...
plot_pages <- function(wl=31, reg, xa='n',box='n',seas='yrmean',anom=T,anomper=(1901:1980),lw=1,
                       sca=F,yl="Temperature [ºC]",xl='Year',plotbem=T,plotmem=T,plotech=T,
  ...
  if (seas=='yrmean') {
    ...
    #  plot(period,eindbem,ty='l',col=rgb(0,0,0,10,maxColorValue=10), ylab="Temperature [ºC]",
  ...
  #  plot(period,allind[,1],ty='l',col=rgb(0,0,0,10,maxColorValue=10), ylab="Temperature [ºC]",
